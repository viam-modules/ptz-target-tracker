...
Examples:
  ...
  %(prog)s --num-poses 18 --ee-clearance 100 --reach 1700
    ...
    parser.add_argument(
        ...
        type=int,
        default=24,
        help='Number of poses to generate (9-16). Min 9: lower layer only. Max 16: lower + high layer + 4 fully extended (default: 24)'
    )
    ...
    elif num_poses_requested > 16:
        print(f"Warning: Maximum is 16 poses. Setting to 16.")
        num_poses_requested = 16
    ...
    # High layer gets all remaining poses (3x1 grid for up to 3 poses)
    ...
    if remaining > 0:
        num_high = remaining
        # High layer: 3x1 grid for up to 3 poses at maximum height
        nx_high, ny_high = 3, 1
        if num_high > 3:
            num_high = 3  # Cap at 3 poses for high layer
    else:
        num_high = 0
        nx_high, ny_high = 0, 0
